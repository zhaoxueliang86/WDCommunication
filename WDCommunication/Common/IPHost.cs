...
namespace WDCommunication.Common
{
    /// <summary>
    /// IP解析映射
    /// <para>
    ...
    public class IPHost
    {
        ...
        public IPHost(string host)
        {
            if (SocketUtility.IsURL(host))
            {
                ...
                if (SocketUtility.IsIPv4(Uri.Host) || SocketUtility.IsIPV6(Uri.Host))
                {
                ...
                else
                {
                    ...
                    {
                        if (addresses != null) Analysis(addresses[0].ToString(), Uri.Port.ToString());
                    }
                }
            }
            else
        ...
        private void Analysis(string ip, string port)
        {
            try
            {
                ...
                IPEndPoint endPoint = new(IPAddress.Parse(ip), portNum);
                if (ip.Contains(':'))
                {
